...
def init():
    filename_pmap = './datafile/density/density10.txt'
...
def animate(n):
    ...
    n100 = (n+1)*100
    filename_pmap = './datafile/density/density' + str(n100) + '.txt'
    data = np.loadtxt(filename_pmap)
    x = data[:,0]
    y = data[:,1]
    z = data[:,2]
...
def colormap(step):
    filename = './datafile/density/density' + str(step) + '.txt'
    ...
    X, Y, Z = grid(x, y, z)
    # 等高線図の生成。
    cont=plt.contour(X,Y,Z,10,colors=['black'])
    #cont.clabel(fmt='%1.1f', fontsize=14)
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.pcolormesh(X,Y,Z,cmap='Spectral_r') #カラー等高線図
    ...
    plt.text(20,7.5,txt)
    print("n:",step)
    filename_save = "./fig/density" + str(step) + ".png"
